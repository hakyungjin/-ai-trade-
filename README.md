...
| `npm run backend` | 백엔드만 실행 (포트 8000) |
| `npm run frontend` | 프론트엔드만 실행 (포트 5173) |
| `npm run install:all` | 모든 의존성 설치 |
| `npm run backend:install` | 백엔드 Python 패키지만 설치 |
| `npm run frontend:install` | 프론트엔드 npm 패키지만 설치 |

### 🗄️ 데이터베이스 마이그레이션 (루트에서 실행)

| 명령어 | 설명 |
|--------|------|
| `npm run migration:upgrade` | 모든 마이그레이션 적용 |
| `npm run migration:downgrade` | 이전 버전으로 롤백 |
| `npm run migration:new "설명"` | 새 마이그레이션 생성 (모델 변경 후) |

---

## 수동 설정 (npm 스크립트 미사용 시)
